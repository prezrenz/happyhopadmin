...
export const getUsersByRole = (users: {}[], role: string) => {
    return users.filter(
        (user: any) => user?.role?.toLowerCase() == role
    );
};

export const getUsersByVerification = (
    users: {}[],
    isVerified: boolean
) => {
    return users.filter(
        (user: any) => user?.verified == isVerified
    );
};
...
export const getAllVerificationRequests = (
    setVerificationRequests: (arg0: {}[]) => void
) => {
    const q = query(
...
export const getVerificationRequestById = (
    requests: {}[],
    id: string
) => {
...
export const verifySupplierUserById = async (
    id: string
) => {
    const user = doc(db, "users", id);
...
export const unverifyVetUserById = async (
    id: string
) => {
    const user = doc(db, "users", id);
...
export const unverifySupplierUserById = async (
    id: string
) => {
    const user = doc(db, "users", id);
...
export const getReportsByHandling = (
    reports: {}[],
    isHandled: boolean
) => {
    return reports.filter(
        (report: any) =>
            (report?.handled ? true : false) ==
            isHandled
    );
};
...
export const getPinsById = (
    pins: {}[],
    id: string
) => {
    return pins.filter(
        (pin: any) => pin?.vetId == id
    );
};
...
export const updatePinById = async (
    id: string,
    data: {
        clinicName?: string;
        latitude?: number;
...
export const getAllVetRecommendations = (
    setRecommendations: (arg0: {}[]) => void
) => {
    const q = query(
        collection(db, "vetRecommendations")
    );
...
export const getVetRecommendationById = (
    recommendations: {}[],
    id: string
) => {
...
export const approveVetRecommendation = async (
    id: string
) => {
...
export const deleteVetRecommendation = async (
    id: string
) => {
    const recommendation = doc(
        db,
...
export const getAllVetImageSubmissions = (
    setSubmissions: (arg0: {}[]) => void
) => {
    const q = query(
        collection(db, "vetImageSubmissions")
    );
...
export const getVetImageSubmissionById = (
    submissions: {}[],
    id: string
) => {
...
export const deleteVetImageSubmissionById = async (
    id: string
) => {
